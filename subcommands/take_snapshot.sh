...
do
	...
	LOCAL_PORT=`jq -r ".application_list.$APPLICATION.local_port" < ~/.ksctl/ksctl.env.json`
	REMOTE_PORT=`jq -r ".application_list.$APPLICATION.remote_port" < ~/.ksctl/ksctl.env.json`
	sqlite3 ~/.ksctl/states.db "insert into applications (l_r, l_d, l_port, r_port, application, fqdn) values (
